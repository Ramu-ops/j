...
const ProfileView: React.FC<ProfileViewProps> = ({ 
  ...
  return (
    <div className="space-y-6 md:space-y-10 pb-24">
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-2xl border border-slate-100 relative">
        <button 
          onClick={() => setIsEditing(!isEditing)}
          className="absolute top-6 md:top-8 right-6 md:right-10 text-indigo-600 font-semibold text-[10px] uppercase tracking-widest hover:underline"
        >
        ...
        {isEditing ? (
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 md:gap-10">
            <div className="relative group">
              <img src={user.avatar} className="w-28 h-28 md:w-36 md:h-36 rounded-[2rem] md:rounded-[3rem] object-cover ring-4 md:ring-8 ring-indigo-50 shadow-2xl" alt="Profile" />
              <div className="absolute -bottom-2 -right-2 bg-green-500 text-white p-2 rounded-2xl shadow-xl border-2 md:border-4 border-white">
            ...
            <div className="flex-1 text-center sm:text-left">
              <h1 className="text-3xl md:text-4xl font-semibold text-slate-900 tracking-tighter mb-2">{user.name}</h1>
              <p className="text-slate-600 font-medium mb-6 leading-relaxed max-w-lg text-sm md:text-base">{user.bio || 'Professional worker in the SkillChain Bharat network.'}</p>
              <div className="flex flex-wrap gap-3 justify-center sm:justify-start">
                <div className="bg-indigo-50 px-4 md:px-5 py-2 md:py-2.5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-normal text-indigo-700 border border-indigo-100 uppercase tracking-widest">
                  UID: {user.phoneNumber}
                </div>
                <div className="bg-emerald-50 px-4 md:px-5 py-2 md:py-2.5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-normal text-emerald-700 border border-emerald-100 uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-slate-900 p-8 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden">
           <div className="absolute top-0 right-0 p-6 md:p-8 opacity-10">
              <svg className="w-24 h-24 md:w-32 md:h-32" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 14h-2v-2h2v2zm0-4h-2V7h2v5z" /></svg>
           </div>
           <h3 className="text-indigo-400 text-[9px] md:text-[10px] font-normal uppercase tracking-widest mb-2">{t.availableEarnings}</h3>
           <div className="text-4xl md:text-5xl font-semibold mb-8 md:mb-10 tracking-tighter">₹ {balance.toLocaleString()}</div>
           <button 
             onClick={() => setShowWithdrawModal(true)} 
             className="w-full bg-white text-slate-900 py-4 md:py-5 rounded-2xl font-medium text-xs md:text-sm shadow-xl hover:bg-indigo-50 active:scale-95 transition"
           >
             {t.withdrawToUpiBank}
           </button>
        </div>

        <div className="bg-white p-8 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] border-2 border-slate-100 shadow-xl flex flex-col justify-center">
           <h3 className="text-slate-600 text-[9px] md:text-[10px] font-normal uppercase tracking-widest mb-6">{t.credentialLocker}</h3>
           <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-100">
                <span className="text-[9px] md:text-[10px] font-normal text-slate-600 uppercase">{t.verified}</span>
                <p className="text-2xl md:text-3xl font-semibold text-slate-900">{skills.filter(s => s.status === 'verified').length}</p>
              </div>
              <div className="bg-slate-50 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-100">
                <span className="text-[9px] md:text-[10px] font-normal text-slate-600 uppercase">{t.pending}</span>
                <p className="text-2xl md:text-3xl font-semibold text-slate-900">{skills.filter(s => s.status === 'pending').length}</p>
              </div>
           </div>
           <button 
            onClick={() => setView(AppView.SKILLS)}
            className="mt-6 w-full bg-indigo-50 text-indigo-600 py-4 rounded-2xl font-medium text-xs md:text-sm hover:bg-indigo-100 transition border border-indigo-100"
           >
             {t.addNewCertificate}
           </button>
        </div>
      ...
      {/* Active Work Section */}
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] border-2 border-slate-100 shadow-xl">
        <h3 className="text-2xl md:text-3xl font-semibold text-slate-900 mb-6 md:mb-8 flex items-center">
          <span className="w-1.5 md:w-2 h-6 md:h-8 bg-indigo-600 rounded-full mr-4 md:mr-5" />
          {t.activeAssignments}
        </h3>
        {activeJobs.length === 0 ? (
          <div className="py-16 md:py-20 text-center bg-slate-50 rounded-[2rem] md:rounded-[3rem] border-2 border-dashed border-slate-200">
             <p className="text-slate-500 font-normal uppercase tracking-widest text-xs md:text-sm">{t.noActiveAssignments}</p>
          ...
          <div className="grid gap-4 md:gap-6">
            {activeJobs.map(job => (
              <div key={job.id} className={`p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 flex flex-col sm:flex-row justify-between items-center gap-6 md:gap-8 group transition-all ${job.status === 'rejected' ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-200 hover:border-indigo-400'}`}>
                 <div className="flex-1 text-center sm:text-left">
                    <h4 className="font-normal text-xl md:text-2xl text-slate-900 mb-2">{job.title}</h4>
                    <div className="flex flex-wrap justify-center sm:justify-start gap-2 mb-4">
                       <span className="text-[8px] md:text-[9px] font-normal text-indigo-900 uppercase tracking-widest bg-indigo-100 px-2 md:px-3 py-1 rounded-lg border border-indigo-200">{job.location}</span>
                       <span className="text-[8px] md:text-[9px] font-normal text-emerald-900 uppercase tracking-widest bg-emerald-100 px-2 md:px-3 py-1 rounded-lg border border-emerald-200">
                         {job.paymentMethod === 'escrow' ? t.verifiedPayout : 'Direct Payout'}
                       </span>
                       <span className="text-[8px] md:text-[9px] font-normal text-slate-900 uppercase tracking-widest bg-slate-200 px-2 md:px-3 py-1 rounded-lg border border-slate-300">
                         Via {job.payoutType?.toUpperCase() || 'WALLET'}
                       </span>
                    </div>
                    <p className="text-xl md:text-2xl font-semibold text-slate-900">{job.budget}</p>
                    {job.status === 'rejected' && (
