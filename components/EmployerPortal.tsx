  ...
  return (
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h2 className="text-3xl md:text-4xl font-semibold text-slate-900 tracking-tight">Employer Hub</h2>
          <p className="text-slate-500 font-medium text-sm md:text-base">Post work and verify talent in your local area.</p>
        </div>
        <button 
          onClick={() => setShowPostModal(true)}
          className="w-full sm:w-auto bg-emerald-600 text-white px-8 py-4 rounded-2xl font-medium shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition"
        >
          Post New Work
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-xl">
           <h3 className="text-lg md:text-xl font-semibold text-slate-900 mb-6">Payment Escrow</h3>
           <div className="p-4 md:p-6 bg-slate-50 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 flex items-center justify-between mb-6">
              <div>
                <p className="text-[9px] md:text-[10px] font-normal text-slate-500 uppercase tracking-widest">Active Escrow Balance</p>
                <p className="text-2xl md:text-3xl font-semibold text-slate-900">₹ {employerBalance.toLocaleString()}</p>
              </div>
              <button 
                onClick={() => setShowTopUpModal(true)}
                className="bg-white p-3 md:p-4 rounded-xl md:rounded-2xl shadow-sm border border-slate-100 text-emerald-600 hover:text-emerald-700 font-medium text-xs md:text-sm"
              >
                Top Up
              </button>
           </div>
           <p className="text-[9px] md:text-[10px] text-slate-400 italic">Funds are automatically released by Verifiers once work is approved.</p>
        </div>

        <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-xl">
           <h3 className="text-lg md:text-xl font-semibold text-slate-900 mb-6">Verification Insights</h3>
           <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                 <span className="text-xs md:text-sm font-medium text-slate-500">Local Workers Available</span>
                 <span className="font-semibold text-slate-900">2,481</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl">
                 <span className="text-xs md:text-sm font-medium text-slate-500">Average Job Match</span>
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3rem] border border-slate-100 shadow-xl">
        <h3 className="text-xl md:text-2xl font-semibold text-slate-900 mb-6 md:mb-8">Completed Work & Reviews</h3>
        <div className="space-y-6">
          {gigs.filter(g => g.status === 'completed').map(gig => (
            <div key={gig.id} className="p-6 md:p-8 bg-emerald-50/20 rounded-[2rem] md:rounded-[2.5rem] border border-emerald-100">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="font-semibold text-lg md:text-xl text-slate-900">{gig.title}</h4>
                  <p className="text-[10px] md:text-xs text-slate-500">Worker: {gig.workerId} • {gig.budget}</p>
                </div>
                <span className="bg-emerald-100 text-emerald-700 text-[8px] md:text-[10px] font-bold px-3 md:px-4 py-1.5 rounded-full uppercase tracking-widest whitespace-nowrap">Completed</span>
              </div>
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-xl">
        <h3 className="text-xl md:text-2xl font-semibold text-slate-900 mb-6 md:mb-8">Your Active Gigs</h3>
        <div className="space-y-4 md:space-y-6">
          {gigs.filter(g => g.status === 'open').map(gig => (
            <div key={gig.id} className="p-5 md:p-6 bg-slate-50 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                <h4 className="text-base md:text-lg font-semibold text-slate-900">{gig.title}</h4>
                <p className="text-[10px] md:text-xs text-slate-500 font-medium">{gig.location} • Budget: ₹{gig.budgetNum}</p>
              </div>
              <div className="flex items-center justify-between md:justify-end space-x-4">
                <div className="text-right">
                  <p className="text-[8px] md:text-[10px] font-normal text-slate-400 uppercase tracking-widest">Applicants</p>
                  <p className="text-base md:text-lg font-semibold text-indigo-600">{gig.applicants?.length || 0}</p>
                </div>
                <button 
                  onClick={() => setSelectedGigForApplicants(gig)}
                  className="bg-white px-4 md:px-6 py-2.5 md:py-3 rounded-xl border border-slate-200 text-slate-900 font-medium text-[10px] md:text-xs hover:bg-slate-100 transition"
                >
                  Review Applicants
                </button>
              </div>
