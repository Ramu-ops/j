  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-24">
      <header className="px-2 md:px-0">
        <h2 className="text-3xl md:text-4xl font-semibold text-slate-900 tracking-tighter mb-2">{t.gigMarketplace}</h2>
        <p className="text-slate-500 font-medium text-sm md:text-base">{t.marketplaceSubtitle}</p>
      </header>

      {/* Advanced Filter Panel */}
      <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 border-slate-100 shadow-xl space-y-6 md:space-y-8">
        {/* Row 1: Search */}
      ...
      <div className="grid gap-6">
        ...
        {!isLoading && filteredGigs.length === 0 ? (
          ...
          !isLoading && filteredGigs.map(gig => {
            ...
            return (
              <div key={gig.id} className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] border-2 border-slate-100 shadow-xl hover:shadow-2xl hover:border-indigo-400 transition-all group overflow-hidden relative">
                <div className="flex flex-col md:flex-row justify-between gap-6 md:gap-8">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 md:space-x-4 mb-4">
                      <span className="bg-emerald-100 text-emerald-900 text-[9px] md:text-[10px] font-normal px-3 md:px-4 py-1.5 rounded-full uppercase border border-emerald-200">{t.openTask}</span>
                      {match && <span className="bg-indigo-900 text-white text-[9px] md:text-[10px] font-normal px-3 md:px-4 py-1.5 rounded-full shadow-lg">{match.matchScore}% Match</span>}
                    </div>
                    <h3 className="text-2xl md:text-3xl font-semibold text-slate-900 mb-2 leading-tight tracking-tight">{gig.title}</h3>
                    <div className="flex items-center text-slate-900 font-semibold text-lg md:text-xl mb-6">
                      <div className="bg-indigo-50 text-indigo-600 p-2 rounded-xl mr-3">
                        <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
                      </div>
                      <span className="truncate">{gig.location}</span> <span className="mx-2 md:mx-3 text-slate-200">|</span> <span className="text-indigo-600 whitespace-nowrap">{gig.budget}</span>
                    </div>
                    <p className="text-slate-600 text-sm font-medium leading-relaxed mb-6 md:mb-8 max-w-2xl">{gig.description}</p>
                    <div className="flex flex-wrap gap-2">
                       {gig.requiredSkills.map(s => <span key={s} className="bg-slate-50 text-slate-800 text-[9px] md:text-[10px] font-normal px-3 md:px-4 py-1.5 md:py-2 rounded-xl border border-slate-200 uppercase tracking-widest">{s}</span>)}
                    </div>
                  </div>
                  <div className="flex flex-col justify-end">
                    <button 
                      onClick={() => handleApplyClick(gig)}
                      className="bg-slate-900 text-white px-8 md:px-12 py-4 md:py-5 rounded-2xl font-medium text-base md:text-lg hover:bg-indigo-700 transition-all shadow-xl active:scale-95 group-hover:scale-105"
                    >
                      {t.acceptGig}
                    </button>
                  </div>
