  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-24">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 tracking-tight">Verification Node</h2>
          <p className="text-gray-500 font-medium text-sm md:text-base">Protecting the local workforce ledger.</p>
        </div>
        <div className="flex bg-white p-1 rounded-2xl border border-gray-100 shadow-sm w-full md:w-fit overflow-x-auto no-scrollbar">
           <button 
            onClick={() => setActiveTab('skills')}
            className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-[10px] md:text-xs font-semibold transition-all whitespace-nowrap ${activeTab === 'skills' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-900'}`}
           >
             CREDENTIALS
           </button>
           <button 
            onClick={() => setActiveTab('work')}
            className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-[10px] md:text-xs font-semibold transition-all whitespace-nowrap ${activeTab === 'work' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-900'}`}
           >
             WORK SUBMISSIONS
           </button>
           <button 
            onClick={() => setActiveTab('problems')}
            className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-[10px] md:text-xs font-semibold transition-all whitespace-nowrap ${activeTab === 'problems' ? 'bg-amber-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-900'}`}
           >
             COMMUNITY ISSUES
           </button>
        </div>
      ...
      {activeTab === 'skills' ? (
        <div className="space-y-4">
          <h3 className="text-base md:text-lg font-semibold text-gray-900 uppercase tracking-widest flex items-center">
            <div className="w-2 md:w-3 h-2 md:h-3 bg-purple-500 rounded-full mr-3" />
            Skill Review Queue ({pendingSkills.length})
          </h3>
          {pendingSkills.map(skill => (
            <div key={skill.id} className="bg-white p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:border-purple-200 transition-all">
              <div className="flex-1">
                <h4 className="font-semibold text-base md:text-lg text-gray-900">{skill.title}</h4>
                <p className="text-[10px] md:text-xs font-medium text-gray-400 uppercase">{skill.issuer}</p>
                <button onClick={() => setSelectedSkill(skill)} className="text-purple-600 font-semibold text-[9px] md:text-[10px] uppercase mt-2 hover:underline">Inspect Proof</button>
              </div>
              <div className="flex space-x-2 w-full md:w-auto">
                <button onClick={() => onVerifySkill(skill.id, 'rejected')} className="flex-1 md:flex-none px-4 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-semibold text-red-600 bg-red-50">Reject</button>
                <button onClick={() => onVerifySkill(skill.id, 'verified')} className="flex-1 md:flex-none px-4 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-semibold text-white bg-purple-600 shadow-lg shadow-purple-100">Verify</button>
              </div>
            </div>
          ))}
          {pendingSkills.length === 0 && <p className="text-center py-16 md:py-20 text-gray-400 font-medium text-sm">No skills pending.</p>}
        </div>
      ) : activeTab === 'work' ? (
        <div className="space-y-4">
          <h3 className="text-base md:text-lg font-semibold text-gray-900 uppercase tracking-widest flex items-center">
            <div className="w-2 md:w-3 h-2 md:h-3 bg-indigo-500 rounded-full mr-3" />
            Work Verification Feed ({pendingWork.length})
          </h3>
          {pendingWork.map(job => (
            <div key={job.id} className="bg-white p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:border-indigo-200 transition-all">
              <div className="flex-1">
                <h4 className="font-semibold text-base md:text-lg text-gray-900">{job.title}</h4>
                <p className="text-[10px] md:text-xs font-medium text-indigo-600 uppercase">Payout: {job.budget} • Via {job.payoutType?.toUpperCase()}</p>
                <button onClick={() => setSelectedWork(job)} className="text-indigo-600 font-semibold text-[9px] md:text-[10px] uppercase mt-2 hover:underline">View Work Proof Photo</button>
              </div>
              <div className="flex space-x-2 w-full md:w-auto">
                <button onClick={() => { setSelectedWork(job); setShowRejectionInput(true); }} className="flex-1 md:flex-none px-4 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-semibold text-red-600 bg-red-50">Decline</button>
                <button onClick={() => onVerifyWork(job.id, true)} className="flex-1 md:flex-none px-4 md:px-6 py-3 rounded-xl text-[10px] md:text-xs font-semibold text-white bg-indigo-600 shadow-lg shadow-indigo-100">Approve & Pay</button>
              </div>
            </div>
          ))}
          {pendingWork.length === 0 && <p className="text-center py-16 md:py-20 text-gray-400 font-medium text-sm">No work submissions pending.</p>}
        </div>
      ) : activeTab === 'problems' ? (
        <div className="space-y-4">
          <h3 className="text-base md:text-lg font-semibold text-gray-900 uppercase tracking-widest flex items-center">
            <div className="w-2 md:w-3 h-2 md:h-3 bg-amber-500 rounded-full mr-3" />
          ...
          {problems.filter(p => p.status === 'pending').sort((a, b) => b.votes - a.votes).map(problem => (
            <div key={problem.id} className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1">
                <div className="flex items-center space-x-3 mb-2">
                  <span className="bg-amber-100 text-amber-700 text-[8px] md:text-[10px] font-bold px-2 md:px-3 py-1 rounded-full">{problem.votes} VOTES</span>
                  <h4 className="font-semibold text-base md:text-lg text-gray-900">{problem.title}</h4>
                </div>
                <p className="text-xs md:text-sm text-gray-500 line-clamp-2">{problem.description}</p>
              </div>
              <button 
                onClick={() => onPromoteProblem(problem.id)}
                className="w-full md:w-auto px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl text-[10px] md:text-xs font-bold uppercase tracking-widest text-white bg-amber-600 shadow-xl shadow-amber-100 hover:bg-amber-700 transition"
              >
                Promote to Employers
              </button>
            </div>
          ))}
          {problems.filter(p => p.status === 'pending').length === 0 && <p className="text-center py-16 md:py-20 text-gray-400 font-medium text-sm">No new problems reported.</p>}
        </div>
      ) : null}
